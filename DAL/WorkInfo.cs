...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number(0 over(order by uploadTime desc) as num,workinfo.* from workinfo where activityId in @activityIds)";
                sql += "sleect * from a where num between (@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
                return cn.Query<Model.WorkInfoNo>(sql, new { pageIndex = page.pageIndex, pageSize = page.pageSizeactivityIds = page.activityIds });
